...
export default function ReportesPage() {
  ...
  const empleadosFiltrados = useMemo(() => {
    ...
    return empleados.filter((emp) => {
      ...
      const empresaNombre = emp.empresa;
  ...
  const textoEmpresaKPI = useMemo(() => {
    ...
    const empresasEnVista = Array.from(
      new Set(empleadosFiltrados.map((e) => e.empresa))
    );
    ...
    <div className="p-8 bg-gray-50 min-h-screen space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow-sm transition-all duration-200 text-sm"
        >
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {empleadosFiltrados.length > 0 ? (
                empleadosFiltrados.map((emp) => {
                  ...
                  return (
                    <tr key={emp.empleadoId} className="hover:bg-blue-50/40 transition-colors">
